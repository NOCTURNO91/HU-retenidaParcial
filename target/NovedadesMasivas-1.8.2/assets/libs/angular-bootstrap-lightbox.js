...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><div class=lightbox-nav><button class=close aria-hidden=true ng-click=$dismiss()>×</button><div class=btn-group ng-if=\"Lightbox.images.length > 1\"><a class=\"btn btn-xs btn-default\" ng-click=Lightbox.prevImage()>‹ Anterior</a> <a ng-href={{Lightbox.imageUrl}} target=_blank class=\"btn btn-xs btn-default\" title=\"Abrir en nueva pestaña\">Abrir en nueva pestaña</a> <a class=\"btn btn-xs btn-default\" ng-click=Lightbox.nextImage()>Siguiente ›</a></div></div><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.imageCaption}}</span></div><img ng-if=!Lightbox.isVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}}><div ng-if=Lightbox.isVideo(Lightbox.image) class=\"embed-responsive embed-responsive-16by9\"><video ng-if=!Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} controls autoplay></video><embed-video ng-if=Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} ng-href={{Lightbox.imageUrl}} iframe-id=lightbox-video class=embed-responsive-item><a ng-href={{Lightbox.imageUrl}}>Watch video</a></embed-video></div><div class=\"alert alert-info\" style=\"margin: 1em 0 0;\">{{Lightbox.imageObservations}}</div></div></div>"
  );
...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.getImageObservations = function (image) {
    return image.imageObservations || "";
  };
  ...
  this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader',
    ...
    Lightbox.getImageObservations = this.getImageObservations;
    ...
    Lightbox.openModal = function (newImages, newIndex, modalParams) {
      ...
      Lightbox.modalInstance.result['finally'](function () {
        ...
        Lightbox.imageObservations = null;
    ...
    Lightbox.setImage = function (newIndex) {
      ...
      var success = function (properties) {
        ...
        Lightbox.imageObservations = properties.imageObservations || Lightbox.getImageObservations(image);
